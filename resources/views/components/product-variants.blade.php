    ...
    <div class="flex items-center space-x-3">
      @if (count($variants) > 0)
        <img id="selectedImage" src="" alt=""
          class="hidden w-10 h-10 object-cover border-2 border-primary rounded-full">
      ...
      @else
        <img id="selectedImage" src="{{ $baseImage }}" alt=""
          class="w-10 h-10 object-cover border-2 border-primary rounded-full">
...
<select id="nativeSelect" name="variant_id" class="hidden">
  ...
  <option {{ count($variants) > 0 ? '' : 'selected' }} value="{{ $baseId }}">{{ $baseName }}</option>
  @foreach ($variants as $variant)
