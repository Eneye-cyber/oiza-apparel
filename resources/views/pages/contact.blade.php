...
@section('description', 'Get in touch with Oiza Apparels for inquiries, support, or feedback. Reach us via phone, email,
  or our contact form.')
@section('og_title', 'Contact Oiza Apparels')
@section('og_description', 'Contact Oiza Apparels for support, inquiries, or feedback about our high-quality fabrics and
  apparel collections.')
        ...
        <div class="flex flex-col justify-center gap-6">
          <div class="">
            ...
            <p class="text-base text-black opacity-80 mt-4 lg:max-w-xl text-justify">
              Have a question or need assistance? We're here to help with your fabric and apparel needs. Reach out via the
              form below or our contact details.
              <br>
              <br>
              Whether you have questions about our fabric collections, need assistance with an order, or simply want to
          ...
          <form action="{{ route('contact.submit') }}" method="POST" class="space-y-6">
            ...
            <div class="grid sm:grid-cols-2 gap-3">
              <div>
                ...
                <input type="text" id="name" name="name"
                  ...
                  placeholder="Your Name" value="{{ old('name') }}" required aria-describedby="name-error">
                @error('name')
              ...
              <div>
                ...
                <input type="email" id="email" name="email"
                  class="mt-2 w-full border border-black rounded-md py-3 px-4 text-black focus:outline-none focus:border-[#555]"
                  placeholder="Your Email" value="{{ old('email') }}" required aria-describedby="email-error">
                @error('email')
            ...
            <div>
              ...
              <input type="text" id="subject" name="subject"
                class="mt-2 w-full border border-black rounded-md py-3 px-4 text-black focus:outline-none focus:border-[#555]"
                placeholder="Subject" value="{{ old('subject') }}" required aria-describedby="subject-error">
              @error('subject')
                <span id="subject-error" class="text-red-600 text-sm mt-1">{{ $message }}</span>
              @enderror
            </div>
            ...
            <div>
              ...
              <textarea id="message" name="message" rows="3"
                class="mt-2 w-full border border-black rounded-md py-3 px-4 text-black focus:outline-none focus:border-[#555]"
                placeholder="Your Message" required aria-describedby="message-error">{{ old('message') }}</textarea>
              @error('message')
                <span id="message-error" class="text-red-600 text-sm mt-1">{{ $message }}</span>
              @enderror
            </div>
            <button type="submit" class="btn-solid uppercase">
              Send Message
            </button>
          </form>
        </div>
    ...
    <div class="container py-16 border-y border-black/35 space-y-6">
      ...
      <div class="flex flex-center gap-4 mt-2">
        <a href="https://www.facebook.com/oiza.awomokun" target="_blank" rel="noopener noreferrer"
          class="text-black hover:text-[#555]" aria-label="Follow Oiza Apparels on Facebook">
        ...
        <a href="https://www.instagram.com/oiza_apparel/" target="_blank" rel="noopener noreferrer"
          class="text-black hover:text-[#555]" aria-label="Follow Oiza Apparels on Instagram">
            ...
            <div class="col-span-1 md:col-span-2 text-center py-12">
              <div class="bg-cream border border-gold rounded-lg p-8 max-w-2xl mx-auto">
                ...
                <p class="text-black opacity-80">
                  We currently don’t have any frequently asked questions to display. Please check back later or contact us
                  for assistance.
                </p>
